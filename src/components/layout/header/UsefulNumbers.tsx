...
const UsefulNumbers: React.FC = () => {
    const numbers = [
        { label: 'Polícia Militar', value: '190', icon: 'fa-shield-halved', color: 'text-blue-500' },
    ...
    return (
        <div className="w-full bg-zinc-950 border-t border-white/5 py-1 px-4 md:px-8 overflow-hidden">
            <div className="max-w-[1600px] mx-auto flex items-center justify-center gap-4 md:gap-8 overflow-x-auto scrollbar-hide py-0.5">
                <span className="text-[9px] font-black text-gray-500 uppercase tracking-widest whitespace-nowrap flex items-center gap-2">
                    <span className="w-1.5 h-1.5 bg-red-600 rounded-full animate-pulse"></span>
                    Números Úteis:
                </span>
                {numbers.map((item, idx) => (
                    <div key={idx} className="flex items-center gap-2 whitespace-nowrap group cursor-pointer hover:scale-105 transition-transform">
