...
const UsersTab: React.FC<UsersTabProps> = ({ allUsers, newsHistory, currentUser, onUpdateUser, onAddUser, onDeleteUser, adConfig, systemSettings, darkMode = true }) => {
    ...
    return (
        <div className="animate-fadeIn relative w-full h-full">
            <header className="mb-8 flex flex-col md:flex-row justify-between items-end gap-4">
                <div>
                    <h1 className={`text-3xl md:text-4xl font-black uppercase italic tracking-tighter mb-2 ${darkMode ? 'text-white' : 'text-gray-900'}`}>GESTÃO DE <span className="text-red-600">EQUIPE</span></h1>
                    <div className="flex items-center gap-4">
                        <p className={`text-xs font-bold uppercase tracking-widest ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>Controle de acesso e auditoria</p>
                        {totalPending > 0 && (
