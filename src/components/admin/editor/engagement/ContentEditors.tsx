...
export const TimelineEditor = ({ settings, style, theme, onChange }: SubEditorProps) => {
    const activeTheme = theme || { classes: { accent: 'bg-blue-500', text: 'text-zinc-600', wrapper: 'bg-white', secondary: 'border-zinc-200' } };
    const events = (settings.timelineEvents as any[]) || [{ date: '2024', title: 'Início', description: 'Descrição aqui' }];
    ...
    const removeEvent = (idx: number) => onChange({ timelineEvents: events.filter((_val: any, i: number) => i !== idx) });
...
export const AccordionEditor = ({ settings, style, theme, onChange }: SubEditorProps) => {
    const activeTheme = theme || { classes: { accent: 'bg-blue-500', text: 'text-zinc-600', wrapper: 'bg-white', secondary: 'border-zinc-200' } };
    const items = (settings.accordionItems as any[]) || [{ title: 'Pergunta 1', content: 'Resposta 1' }];
    ...
    const removeItem = (idx: number) => onChange({ accordionItems: items.filter((_val: any, i: number) => i !== idx) });
