        ...
        <div>
            <div className="grid grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input value={s.leftLabel || '👎'} onChange={e => onChange({ leftLabel: e.target.value })} className={`w-full border-none rounded-lg px-3 py-2 ${activeTheme.classes.secondary.replace('border-', 'bg-').replace('200', '50')} ${activeTheme.classes.text}`} />
                </div>
                <div>
                    <label className={`block text-[9px] font-black uppercase mb-1 opacity-60 ${activeTheme.classes.text}`}>Emoji Direita</label>
                    <input value={s.rightLabel || '👍'} onChange={e => onChange({ rightLabel: e.target.value })} className={`w-full border-none rounded-lg px-3 py-2 ${activeTheme.classes.secondary.replace('border-', 'bg-').replace('200', '50')} ${activeTheme.classes.text}`} />
                </div>
            </div>
            ...
            <div className={`p-4 rounded-xl border ${activeTheme.classes.wrapper} transition-colors`}>
                ...
                <div className="flex items-center gap-3">
                    <span className="text-xl filter grayscale opacity-80">{s.leftLabel || '👎'}</span>
                    ...
                    <span className="text-xl filter grayscale opacity-80">{s.rightLabel || '👍'}</span>
                </div>
            </div>
        </div>
    );
...
export const ReactionEditor = ({ settings, style, theme, onChange }: SubEditorProps) => {
    const s = settings as Record<string, unknown>;
    const activeTheme = theme || { classes: { accent: 'bg-blue-500', text: 'text-zinc-600', wrapper: 'bg-white', secondary: 'border-zinc-200' } };
    const emojis = (s.reactionEmojis || '👍,❤️,😂,😮,😢,😡').split(',');
        ...
        <div>
            <div className="grid grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input value={s.imageA || ''} onChange={e => onChange({ imageA: e.target.value })} className={`w-full border-none rounded-lg px-3 py-2 text-xs mb-2 ${activeTheme.classes.secondary.replace('border-', 'bg-').replace('200', '50')} ${activeTheme.classes.text}`} type="url" placeholder="https://..." />
                    <input value={s.labelA || 'Opção A'} onChange={e => onChange({ labelA: e.target.value })} className={`w-full border-none rounded-lg px-3 py-2 ${activeTheme.classes.secondary.replace('border-', 'bg-').replace('200', '50')} ${activeTheme.classes.text}`} placeholder="Nome A" />
                </div>
                <div>
                    <label className={`block text-[9px] font-black uppercase mb-1 opacity-60 ${activeTheme.classes.text}`}>Opção B (URL Imagem)</label>
                    <input value={s.imageB || ''} onChange={e => onChange({ imageB: e.target.value })} className={`w-full border-none rounded-lg px-3 py-2 text-xs mb-2 ${activeTheme.classes.secondary.replace('border-', 'bg-').replace('200', '50')} ${activeTheme.classes.text}`} type="url" placeholder="https://..." />
                    <input value={s.labelB || 'Opção B'} onChange={e => onChange({ labelB: e.target.value })} className={`w-full border-none rounded-lg px-3 py-2 ${activeTheme.classes.secondary.replace('border-', 'bg-').replace('200', '50')} ${activeTheme.classes.text}`} placeholder="Nome B" />
                </div>
            </div>
            ...
            <div className={`p-4 rounded-xl border ${activeTheme.classes.wrapper} transition-colors grid grid-cols-2 gap-2 text-center`}>
                <div className={`p-3 rounded-lg border-2 ${activeTheme.classes.secondary} border-dashed flex flex-col items-center justify-center aspect-square`}>
                    ...
                    <span className={`text-[10px] font-bold uppercase ${activeTheme.classes.text}`}>{s.labelA || 'Opção A'}</span>
                </div>
                <div className={`p-3 rounded-lg border-2 ${activeTheme.classes.secondary} border-dashed flex flex-col items-center justify-center aspect-square`}>
                    <i className={`fas fa-image text-2xl mb-2 opacity-30 ${activeTheme.classes.text}`}></i>
                    <span className={`text-[10px] font-bold uppercase ${activeTheme.classes.text}`}>{s.labelB || 'Opção B'}</span>
                </div>
            </div>
        </div>
    );
...
export const ThermometerEditor = ({ settings, style, theme, onChange }: SubEditorProps) => {
    ...
    return (
        <div>
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
                <input value={s.minTemp || 'Gelo'} onChange={e => onChange({ minTemp: e.target.value })} className={`w-full border-none rounded-lg px-3 py-2 ${activeTheme.classes.secondary.replace('border-', 'bg-').replace('200', '50')} ${activeTheme.classes.text}`} placeholder="Ex: Gelo" />
                <input value={s.maxTemp || 'Fogo'} onChange={e => onChange({ maxTemp: e.target.value })} className={`w-full border-none rounded-lg px-3 py-2 ${activeTheme.classes.secondary.replace('border-', 'bg-').replace('200', '50')} ${activeTheme.classes.text}`} placeholder="Ex: Fogo" />
            </div>

            {/* Preview */}
            <div className={`p-6 rounded-xl border ${activeTheme.classes.wrapper} transition-colors flex items-center gap-4`}>
                <div className="flex-1 text-right text-xs font-bold opacity-60">{s.minTemp || 'Gelo'}</div>
                ...
                <div className="flex-1 text-left text-xs font-bold opacity-60">{s.maxTemp || 'Fogo'}</div>
            </div>
        </div>
    );
};
        ...
        <div>
            <div className="grid grid-cols-2 gap-4 mb-4">
                <div className={`p-3 rounded-xl border ${activeTheme.classes.secondary} bg-white/50`}>
                    ...
                    <textarea value={s.frontText || ''} onChange={e => onChange({ frontText: e.target.value })} placeholder="Texto Frente" className={`w-full border-none rounded px-2 py-1 text-xs h-20 mb-2 bg-white ${activeTheme.classes.text}`} />
                    <input value={s.frontImage || ''} onChange={e => onChange({ frontImage: e.target.value })} placeholder="URL Imagem" className={`w-full border-none rounded px-2 py-1 text-[10px] bg-white ${activeTheme.classes.text}`} />
                </div>
                <div className={`p-3 rounded-xl border-2 border-stone-800 bg-stone-900`}>
                    ...
                    <textarea value={s.backText || ''} onChange={e => onChange({ backText: e.target.value })} placeholder="Texto Verso" className="w-full bg-stone-800 text-white border-none rounded px-2 py-1 text-xs h-20 mb-2" />
                    <input value={s.backImage || ''} onChange={e => onChange({ backImage: e.target.value })} placeholder="URL Imagem" className="w-full bg-stone-800 text-white border-none rounded px-2 py-1 text-[10px]" />
                </div>
            </div>
