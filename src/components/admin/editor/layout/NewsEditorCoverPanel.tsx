            ...
            <div className="absolute top-4 left-4 z-40 flex flex-col gap-3 pointer-events-auto">
                ...
                {bannerType === 'image' && (
                    <div className="bg-black p-1.5 rounded-2xl flex flex-wrap max-w-[180px] gap-1.5 border border-white/10 shadow-xl animate-fadeIn">
                        {TEMPLATES.map(t => (
                            <button key={t.id} onClick={() => setBannerLayout(t.id as BannerLayout)} className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all ${bannerLayout === t.id ? 'bg-white text-black' : 'text-white/40 hover:text-white'}`} title={t.label}>
                        ...
                        <div className="flex gap-1.5 w-full">
                            {TRANSITIONS.map(tr => (
                                <button key={tr.id} onClick={() => setBannerTransition(tr.id as BannerTransition)} className={`flex-1 h-8 rounded-lg flex items-center justify-center transition-all ${bannerTransition === tr.id ? 'bg-blue-600 text-white' : 'text-white/30 hover:text-white'}`} title={tr.label}><i className={`fas ${tr.icon} text-[10px]`}></i></button>
                            ))}
                        </div>
                    </div>
                )}
