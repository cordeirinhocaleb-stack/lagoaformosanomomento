...
export const renderCurrentText = (variant: string, perStyle: unknown, baseStyles: React.CSSProperties, Tag: unknown, contentRef: unknown, handleInput: () => void) => {
    ...
    if (variant === 'newspaper_standard') {
        const ns = styles.newspaper_standard || { columnWidth: 'full', paragraphGap: 'normal', dropCap: false, dropCapLines: 2, hyphenation: 'off', justify: false };
        variantStyles = {
            ...fluidStyles,
        ...
        const fn = styles.footnote || { prefix: 'none', opacity: 0.7, sizePx: 13, italic: true };
        ...
        const tn = styles.tech_neon || { neonColor: '#00f2ff', glow: 1, panel: 'none', mono: true };
        ...
        const ns = styles.standard_clean || { fontSize: '1.125rem', lineHeight: '1.75' };
        ...
        const ep = styles.editorial_prose || { fontSize: '1.25rem', lineHeight: '1.8' };
        variantStyles = {
            ...fluidStyles,
        ...
        const bb = styles.breaking_brief || { fontSize: '1.125rem', fontWeight: '600' };
