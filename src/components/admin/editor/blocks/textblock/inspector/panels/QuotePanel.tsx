...
export const QuotePanel: React.FC<QuotePanelProps> = ({ variant, data, onChange, darkMode = false }) => {
    ...
    return (
      <div className="space-y-4 animate-fadeIn">
        ...
        <div>
          ...
          <div className={`flex gap-2 flex-wrap items-center p-3 rounded-2xl border ${darkMode ? 'bg-black border-white/5' : 'bg-transparent border-transparent'}`}>
            {[
              ...
              <button
                ...
                className={`w-8 h-8 rounded-full transition-all border-2 ${iq.color === c.val ? (darkMode ? 'border-red-500 ring-2 ring-red-500/20 scale-110 shadow-md' : 'border-white ring-2 ring-zinc-300 scale-110 shadow-md') : 'border-transparent opacity-60 hover:opacity-100'}`}
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <select value={iq.borderPosition} onChange={e => onChange({ borderPosition: e.target.value })} className={`w-full border rounded-lg p-2 text-[10px] font-bold outline-none ${darkMode ? 'bg-black border-white/10 text-white focus:border-red-600' : 'bg-zinc-50 border-zinc-100 text-zinc-900 focus:border-blue-500'}`}>
          ...
          <div>
            ...
            <select value={iq.quoteSize} onChange={e => onChange({ quoteSize: e.target.value })} className={`w-full border rounded-lg p-2 text-[10px] font-bold outline-none ${darkMode ? 'bg-black border-white/10 text-white focus:border-red-600' : 'bg-zinc-50 border-zinc-100 text-zinc-900 focus:border-blue-500'}`}>
        ...
        <div className="flex flex-col gap-3 pt-2">
          <div className="flex items-center justify-between">
            ...
            <input type="range" min="2" max="20" value={iq.borderWidth} onChange={e => onChange({ borderWidth: Number(e.target.value) })} className={`w-24 appearance-none h-1 rounded-lg ${darkMode ? 'bg-zinc-800 accent-red-600' : 'bg-zinc-200 accent-black'}`} />
          </div>
          <div className="flex items-center justify-between">
            <span className="text-[8px] font-black uppercase text-zinc-400">Aspas Gigantes</span>
            <input type="checkbox" checked={iq.bigQuotes} onChange={e => onChange({ bigQuotes: e.target.checked })} className="accent-red-600 w-4 h-4" />
          </div>
          <div className="flex items-center justify-between">
            <span className="text-[8px] font-black uppercase text-zinc-400">Fundo Sutil</span>
            <input type="checkbox" checked={iq.backgroundSubtle} onChange={e => onChange({ backgroundSubtle: e.target.checked })} className="accent-red-600 w-4 h-4" />
          </div>
        </div>

        <div className={`pt-4 border-t ${darkMode ? 'border-white/5' : 'border-zinc-50'}`}>
          <label className="text-[8px] font-black uppercase text-zinc-400 mb-1 block">Autor / Fonte</label>
          <input type="text" value={iq.author || ''} onChange={e => onChange({ author: e.target.value })} className={`w-full border rounded-lg p-2 text-[10px] font-bold outline-none ${darkMode ? 'bg-black border-white/10 text-white focus:border-red-600' : 'bg-zinc-50 border-zinc-100 text-zinc-900 focus:border-blue-500'}`} placeholder="Ex: Welix Duarte" />
          <div className={`flex gap-1 h-7 mt-2 p-1 rounded-lg ${darkMode ? 'bg-black' : 'bg-zinc-100'}`}>
            {['left', 'center', 'right'].map(a => (
              <button key={a} onClick={() => onChange({ authorAlign: a })} className={`flex-1 rounded flex items-center justify-center transition-all ${iq.authorAlign === a ? (darkMode ? 'bg-red-600 text-white shadow-lg' : 'bg-zinc-900 text-white shadow-md') : 'text-zinc-500'}`}><i className={`fas fa-align-${a} text-[10px]`}></i></button>
            ))}
          </div>
        </div>
      </div>
