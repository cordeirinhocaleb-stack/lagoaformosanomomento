...
export const SmartBlockRenderer: React.FC<SmartBlockRendererProps> = ({ block }) => {
    logger.log('\[SmartBlockRenderer\] RENDER: ' + block.id + ' ' + block.settings.widgetId); // ENABLED LOG 
    ...
    useEffect(() => {
        ...
        if (!widgetId) {
            ...
            if (widgetDef) {
                ...
                logger.log('[SmartBlockRenderer] Inferred ID:', widgetId); // ENABLED LOG
            }
        }
        ...
        if (rootElement && activeStyle) {
            ...
            logger.log(`[SmartBlockRenderer] Applying [${activeStyle.id}] to`, rootElement.tagName); // ENABLED LOG
            ...
            logger.error('[SmartBlockRenderer] Root element or style not found', {
                root: !!rootElement,
                style: !!activeStyle,
    ...
    return (
        <div ref={containerRef} dangerouslySetInnerHTML={{ __html: sanitize(block.content as string) }} className="w-full" />
    );
};
