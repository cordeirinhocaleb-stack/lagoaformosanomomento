...
const YouTubeConfigModal: React.FC<YouTubeConfigModalProps> = ({
    videoFile, onConfirm, onCancel, currentTitle, currentDescription, isShorts = false, darkMode = true
}) => {
    ...
    return ReactDOM.createPortal(
        <div className="fixed inset-0 z-[6000] bg-black/90 backdrop-blur-md flex items-center justify-center p-4 animate-fadeIn overflow-y-auto custom-scrollbar">
            <div className={`w-full max-w-2xl rounded-[3rem] overflow-hidden shadow-2xl relative my-8 border ${darkMode ? 'bg-[#0F0F0F] border-white/5' : 'bg-white border-transparent'}`}>
                ...
                <form onSubmit={handleSubmit} className="p-10 space-y-8">
                    ...
                    <div className={`p-4 rounded-2xl flex items-start gap-3 border ${darkMode ? 'bg-blue-900/10 border-blue-900/30' : 'bg-blue-50 border-blue-100'}`}>
                        ...
                        <div>
                            <p className={`text-[10px] font-[1000] uppercase ${darkMode ? 'text-blue-300' : 'text-blue-900'}`}>Processamento Automático</p>
                        ...
                        <div className="space-y-6">
                            <div>
                                ...
                                <input
                                    ...
                                    className={`w-full border-2 rounded-xl px-4 py-3 text-sm font-bold outline-none focus:border-red-500 transition-all placeholder:text-gray-600 ${darkMode ? 'bg-black/40 border-white/5 text-white' : 'bg-gray-50 border-gray-100 text-gray-900'}`}
                            ...
                            <div>
                                ...
                                <textarea
                                    ...
                                    className={`w-full border-2 rounded-xl px-4 py-3 text-sm font-medium outline-none focus:border-red-500 resize-none transition-all placeholder:text-gray-600 ${darkMode ? 'bg-black/40 border-white/5 text-white' : 'bg-gray-50 border-gray-100 text-gray-700'}`}
                        ...
                        <div className="space-y-6">
                            <div>
                                ...
                                <input
                                    ...
                                    className={`w-full border-2 rounded-xl px-4 py-3 text-sm font-bold outline-none focus:border-red-600 transition-all ${darkMode ? 'bg-black/40 border-white/5 text-white' : 'bg-gray-50 border-gray-100 text-gray-900'}`}
                                />
                            </div>
                            ...
                            <div className="grid grid-cols-1 gap-4">
                                <div>
                                    ...
                                    <select
                                        ...
                                        className={`w-full border rounded-xl px-4 py-3 text-sm font-bold outline-none focus:border-red-500 transition-all ${darkMode ? 'bg-black/40 border-white/5 text-white' : 'bg-gray-50 border-gray-100 text-gray-900'}`}
                                    >
                                        {YOUTUBE_CATEGORIES.map(cat => <option key={cat.id} value={cat.id} className={darkMode ? 'bg-zinc-900' : ''}>{cat.name}</option>)}
                                    </select>
                                </div>
                                <div>
                                    ...
                                    <select
                                        ...
                                        className={`w-full border rounded-xl px-4 py-3 text-sm font-bold outline-none focus:border-red-500 transition-all ${darkMode ? 'bg-black/40 border-white/5 text-white' : 'bg-gray-50 border-gray-100 text-gray-900'}`}
                                    >
                                        <option value="public" className={darkMode ? 'bg-zinc-900' : ''}>Público (Visível a todos)</option>
                                        <option value="unlisted" className={darkMode ? 'bg-zinc-900' : ''}>Não Listado (Apenas link)</option>
                                        <option value="private" className={darkMode ? 'bg-zinc-900' : ''}>Privado (Apenas eu)</option>
                                    </select>
                                </div>
                            </div>

                            <div className={`p-5 rounded-2xl border ${darkMode ? 'bg-zinc-900/30 border-white/5' : 'bg-zinc-50 border-zinc-100'}`}>
                                <label className="flex items-start gap-4 cursor-pointer">
                                    ...
                                    />
                                    <div>
                                        <span className={`text-[10px] font-black uppercase tracking-tight block mb-1 ${darkMode ? 'text-white' : 'text-zinc-900'}`}>Conteúdo p/ Crianças?</span>
                    ...
                    <div className={`pt-8 border-t flex flex-col md:flex-row gap-4 justify-end ${darkMode ? 'border-white/5' : 'border-gray-100'}`}>
                        <button type="button" onClick={onCancel} className={`px-8 py-4 rounded-2xl font-black uppercase text-[10px] tracking-widest transition-colors ${darkMode ? 'bg-white/5 text-gray-400 hover:bg-white/10' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}>Cancelar</button>
                        <button
                            type="submit"
