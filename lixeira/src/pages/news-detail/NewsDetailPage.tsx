...
interface NewsDetailProps {
  ...
  onEditMatéria?: (item: NewsItem) => void;
  onDesativarMatéria?: (item: NewsItem) => void;
}
...
const NewsDetailPage: React.FC<NewsDetailProps> = (props) => {
  const { 
    ...
    adConfig, onEditMatéria, onDesativarMatéria
  } = props;
  ...
  return (
    <div className={`bg-white dark:bg-zinc-950 min-h-screen font-sans transition-all duration-500 flex flex-col ${readingMode ? 'reading-focus' : ''}`}>
      ...
      {/* STAFF CONTROL BAR - FLUTUANTE */}
      {canManage && (
          <div className="fixed bottom-24 left-1/2 -translate-x-1/2 z-[1000] animate-fadeIn">
              <div className="bg-black/90 backdrop-blur-xl border-2 border-red-600 rounded-full px-6 py-3 flex items-center gap-6 shadow-2xl">
                  <div className="flex flex-col border-r border-white/20 pr-6">
                      ...
                      <span className="text-white text-[10px] font-bold uppercase truncate max-w-[100px]">{user?.name.split(' ')[0]}</span>
                  </div>
                  
                  <div className="flex items-center gap-3">
                      <button 
                        onClick={() => onEditMatéria?.(news)}
                        className="bg-white text-black hover:bg-red-600 hover:text-white px-4 py-1.5 rounded-full text-[9px] font-[1000] uppercase tracking-widest transition-all flex items-center gap-2 group"
                      >
                          <i className="fas fa-pen-nib group-hover:animate-almost-fall"></i> Editar Direto
                      </button>
                      
                      <button 
                        onClick={() => onDesativarMatéria?.(news)}
                        className="bg-red-600/20 text-red-500 hover:bg-red-600 hover:text-white border border-red-600/30 px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest transition-all flex items-center gap-2"
                      >
                          <i className="fas fa-ban"></i> Desativar Notícia
                      </button>
                  </div>

                  <button 
                    onClick={onBack}
                    className="w-8 h-8 bg-white/10 rounded-full flex items-center justify-center text-white hover:bg-white hover:text-black transition-all"
                    title="Voltar"
                  >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 pt-6 md:pt-10">
            <aside className={`lg:col-span-2 hidden lg:block transition-all duration-500 ${readingMode ? 'opacity-0' : 'opacity-100'}`}>
                <div className="sticky top-28">
                    <LeftAdsRail advertisers={advertisers} onAdvertiserClick={onAdvertiserClick || (() => {})} />
                </div>
            </aside>
            ...
            <main className={`${readingMode ? 'lg:col-span-12 max-w-3xl mx-auto' : 'lg:col-span-7'} space-y-6`}>
                ...
                <div className="py-4">
                    ...
                    {!readingMode && ( <AdvertisersFooter advertisers={advertisers} onAdvertiserClick={onAdvertiserClick || (() => {})} /> )}
                </div>
            </main>
