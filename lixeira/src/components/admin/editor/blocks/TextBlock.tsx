  ...
  const handleMenuAction = (action: string, value?: string) => {
    ...
    switch (action) {
        ...
        case 'link':
            ...
            break;
        case 'align':
            execCommand('justify' + (value === 'justify' ? 'Full' : value?.charAt(0).toUpperCase() + value?.slice(1)));
            break;
        case 'delete':
  ...
  return (
    ...
    >
      ...
      {isSelected && isMobileFormattingOpen && (
          <div className="fixed bottom-[100px] left-4 right-4 z-[1900] animate-slideUp">
      ...
      <div className="w-full relative z-10">
        {renderTextMode(variant, perStyle, globalStyles, Tag, contentRef, handleInput)}
      </div>
      ...
      {/* Desktop Menu de Contexto Personalizado */}
      {contextMenu && (
        <EditorContextMenu 
            x={contextMenu.x} 
            y={contextMenu.y} 
