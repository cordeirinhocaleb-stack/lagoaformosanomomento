...
const GalleryRenderer: React.FC<GalleryRendererProps> = ({ block }) => {
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-4 h-[500px]">
            ...
            <div className="md:col-span-4 flex flex-col gap-4">
                <div className="flex-1 rounded-[2.5rem] overflow-hidden shadow-lg">
                    <img src={items[1]?.url || items[0]?.url} className="w-full h-full object-cover" alt="" />
                </div>
                <div className="flex-1 rounded-[2.5rem] overflow-hidden shadow-lg">
                    <img src={items[2]?.url || items[0]?.url} className="w-full h-full object-cover" alt="" />
                </div>
            </div>
        </div>
    );
    ...
    const renderFilmstrip = () => (
        <div className="space-y-4">
            <div className="aspect-video w-full rounded-[3rem] overflow-hidden shadow-2xl bg-black border border-zinc-100">
                <img src={items[activeIndex].url} className="w-full h-full object-contain" alt="" />
            </div>
    ...
    const renderComparison = () => (
        <div className="relative w-full aspect-video rounded-[3rem] overflow-hidden shadow-2xl group select-none">
            ...
            <img src={items[1]?.url || items[0]?.url} className="absolute inset-0 w-full h-full object-cover" alt="Depois" />
            ...
            >
                <img src={items[0]?.url} className="absolute inset-0 w-full h-full object-cover" alt="Antes" />
            </div>
        ...
        <div className="columns-1 sm:columns-2 lg:columns-3 gap-6 space-y-6">
            {items.map((item) => (
                <div key={item.id} className="break-inside-avoid rounded-3xl overflow-hidden shadow-lg border border-zinc-100 hover:shadow-2xl transition-all group">
                    <img src={item.url} className="w-full h-auto object-cover transition-transform duration-700 group-hover:scale-110" alt="" />
                </div>
            ))}
        </div>
    );
