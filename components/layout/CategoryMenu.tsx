...
const CategoryMenu: React.FC<CategoryMenuProps> = ({ 
  ...
  return (
    <nav className="bg-white pt-4 pb-2 border-b border-gray-100 sticky top-0 md:relative z-[90]">
      <div className="container mx-auto px-4">
        {/* Linha Superior: Título + Botão de Comando Central */}
        <div className="flex items-center justify-between mb-4 gap-4">
          ...
          <div className="flex-grow h-[1px] bg-gray-100 hidden sm:block"></div>
          
          {/* BOTÃO DA CENTRAL - Sempre visível e funcional */}
          <button 
            onClick={onAdminClick}
            className={`flex items-center gap-2 px-4 py-1.5 rounded-full transition-all shadow-lg active:scale-95 shrink-0 ${
              user 
                ? 'bg-red-600 text-white hover:bg-black' 
                : 'bg-black text-white hover:bg-red-600'
            }`}
          >
            <i className={`fas ${user ? 'fa-satellite-dish' : 'fa-lock-open'} text-[10px]`}></i>
            <span className="text-[10px] font-black uppercase tracking-widest whitespace-nowrap">
              {user ? 'Central de Comando' : 'Entrar na Central'}
            </span>
            {user && <span className="w-1.5 h-1.5 bg-white rounded-full animate-pulse ml-1"></span>}
          </button>
        </div>
