  ...
  return (
      ...
      >
        <style>{`
            ...
            @keyframes move-sides { 0%, 100% { transform: scale(1.1) translateX(0); } 25% { transform: scale(1.1) translateX(-5px); } 75% { transform: scale(1.1) translateX(5px); } }
            ...
            .animate-mic-tip { animation: mic-tip 2s ease-in-out 2; }
            .hover-jump:hover { animation: jump-once 0.5s ease-in-out 1; }
            .mic-move:hover { animation: move-sides 0.5s ease-in-out infinite; }
        `}</style>
        ...
        <div className="relative w-full h-full z-30 flex items-center px-4 pointer-events-auto">
            <div className="w-full h-full relative group cursor-pointer" onClick={onHomeClick}>
                <div className="absolute left-[-4%] top-1/2 -translate-y-1/2 w-[35vw] h-[35vw] md:left-[2%] md:w-[30vw] md:h-[30vw] z-20 shrink-0">
                   <img 
                      src={brandIconUrl} 
                      className={`w-full h-full object-contain drop-shadow-[0_1vw_3vw_rgba(0,0,0,0.6)] transition-transform duration-300 mic-move ${micTipping ? 'animate-mic-tip' : ''}`} 
                      onMouseEnter={triggerMicAnimation}
                      alt="Brand Icon"
                   />
                </div>
