...
const NewsTicker: React.FC<NewsTickerProps> = ({ latestNews, brazilNews, onNewsClick }) => {
  ...
  const useTickerAutoScroll = (ref: React.RefObject<HTMLDivElement | null>, isPaused: boolean, manualBlock: boolean, canMove: boolean, speed: number = 1) => {
    useEffect(() => {
  ...
  const scrollManual = (ref: React.RefObject<HTMLDivElement | null>, direction: 'left' | 'right', setManual: (v: boolean) => void, canMove: boolean) => {
    const container = ref.current;
