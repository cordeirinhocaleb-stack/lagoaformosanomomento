...
const FullWidthPromo: React.FC<FullWidthPromoProps> = ({ banners, customHeight }) => {
  const [currentSlide, setCurrentSlide] = useState(0);
  const [backgroundIndex, setBackgroundIndex] = useState(0);
  const videoRef = useRef<HTMLVideoElement>(null);
  
  const activeBanners = (banners && banners.length > 0) ? banners.filter(b => b.active) : DEFAULT_SLIDES;
  ...
  return (
    <section className={`w-full bg-black relative overflow-hidden group shadow-2xl border-y-4 border-red-600 ${customHeight || 'h-[350px] md:h-[500px] my-4 md:my-8'}`}>
      <div className="absolute inset-0 z-0">
