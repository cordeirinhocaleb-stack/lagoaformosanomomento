                ...
                <section className="bg-zinc-900 p-6 rounded-[2.5rem] text-white space-y-6">
                    ...
                    {block.type === 'cta' && (
                        <div className="space-y-4">
                            <div>
                                <label className="text-[8px] font-bold text-zinc-500 uppercase block mb-1">Texto do Botão</label>
                                <input type="text" value={block.content.text} onChange={e => updateContent({ text: e.target.value })} className="w-full bg-white/10 border border-white/10 p-3 rounded-xl text-xs font-bold outline-none focus:border-blue-400" />
                            </div>
                            <div>
                                <label className="text-[8px] font-bold text-zinc-500 uppercase block mb-1">Link (URL)</label>
                                <input type="text" value={block.content.url} onChange={e => updateContent({ url: e.target.value })} className="w-full bg-white/10 border border-white/10 p-3 rounded-xl text-xs font-mono text-blue-300 outline-none" />
                            </div>
                        </div>
                    )}
                    ...
                    {block.type === 'related' && (
                        <div>
                            <label className="text-[8px] font-bold text-zinc-500 uppercase block mb-1">Título da Matéria Relacionada</label>
                            <input type="text" value={block.content.title} onChange={e => updateContent({ title: e.target.value })} className="w-full bg-white/10 border border-white/10 p-3 rounded-xl text-xs font-bold outline-none" />
                        </div>
                    )}
                    ...
                    {block.type === 'table' && (
                        <div className="space-y-4">
                             <div className="flex gap-2">
                                <button onClick={() => {
                                    const rows = [...block.content.rows, new Array(block.content.rows[0].length).fill('-')];
                                    updateContent({ rows });
                                }} className="flex-1 bg-white/10 py-2 rounded-lg text-[8px] font-black uppercase hover:bg-white/20 transition-all">+ Linha</button>
                                <button onClick={() => {
                                    const rows = block.content.rows.map((r: string[]) => [...r, '-']);
                                    updateContent({ rows });
                                }} className="flex-1 bg-white/10 py-2 rounded-lg text-[8px] font-black uppercase hover:bg-white/20 transition-all">+ Coluna</button>
                             </div>
                        </div>
                    )}
