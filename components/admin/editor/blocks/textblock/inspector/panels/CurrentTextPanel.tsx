...
export const CurrentTextPanel: React.FC<CurrentTextPanelProps> = ({ variant, data, onChange }) => {
      ...
      <div className="space-y-6 animate-fadeIn">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <select value={ns.columnWidth} onChange={e => onChange({ columnWidth: e.target.value })} className="w-full bg-white border-2 border-zinc-100 rounded-xl p-3 text-[11px] font-black uppercase outline-none focus:border-red-500">
          ...
          <div>
            ...
            <select value={ns.paragraphGap} onChange={e => onChange({ paragraphGap: e.target.value })} className="w-full bg-white border-2 border-zinc-100 rounded-xl p-3 text-[11px] font-black uppercase outline-none focus:border-red-500">
        ...
        <div className="flex items-center justify-between border-y border-zinc-100 py-4 px-1">
          <div className="flex items-center gap-3">
             <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-sm ${ns.dropCap ? 'bg-red-600 text-white shadow-lg' : 'bg-gray-100 text-gray-400'}`}>
                <i className="fas fa-font"></i>
             </div>
             <span className="text-[10px] font-[1000] uppercase text-zinc-700 tracking-wider">Capitular (Drop Cap)</span>
          </div>
        ...
        {ns.dropCap && (
          <div className="animate-fadeIn bg-red-50/30 p-4 rounded-2xl border border-red-100/50">
            ...
            <div className="flex gap-2 h-10">
              {[2, 3, 4].map(l => (
                <button key={l} onClick={() => onChange({ dropCapLines: l })} className={`flex-1 rounded-xl border-2 text-[11px] font-black transition-all ${ns.dropCapLines === l ? 'bg-red-600 text-white border-red-600 shadow-lg' : 'bg-white text-zinc-400 border-zinc-100'}`}>{l} L</button>
              ))}
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 gap-4">
          <button 
            ...
            className={`flex flex-col items-center gap-2 p-4 rounded-2xl border-2 transition-all ${ns.hyphenation === 'on' ? 'bg-black text-white border-black shadow-lg' : 'bg-white text-gray-400 border-zinc-100'}`}
          >
          ...
          <button 
            ...
            className={`flex flex-col items-center gap-2 p-4 rounded-2xl border-2 transition-all ${ns.justify ? 'bg-black text-white border-black shadow-lg' : 'bg-white text-gray-400 border-zinc-100'}`}
          >
