...
export const HeadlinePanel: React.FC<HeadlinePanelProps> = ({ variant, data, onChange, darkMode = false }) => {
  if (variant === 'hero_headline') {
    ...
    return (
      <div className="space-y-6 animate-fadeIn w-full">
          ...
          <div className="grid grid-cols-3 gap-2 w-full">
            {[
              ...
              <button
                ...
                className={`py-3 rounded-xl border-2 text-[10px] font-black transition-all w-full flex items-center justify-center ${hh.weight === w.val ? (darkMode ? 'bg-white text-black border-white shadow-md' : 'bg-black text-white border-black shadow-md') : (darkMode ? 'bg-transparent text-zinc-600 border-white/5 hover:border-white/10' : 'bg-white text-zinc-400 border-gray-100 hover:bg-gray-50')}`}
              >
                {w.lbl}
              </button>
            ))}
        ...
        <div className={`grid grid-cols-2 gap-4 pt-2 border-t ${darkMode ? 'border-white/5' : 'border-gray-100'}`}>
          <div>
            ...
            <input
              ...
              className={`w-full h-1.5 rounded-lg appearance-none cursor-pointer ${darkMode ? 'bg-zinc-800 accent-red-600' : 'bg-gray-200 accent-black'}`}
            />
          </div>
          <div className="flex flex-col gap-1">
            <label className="text-[8px] font-black uppercase text-zinc-500 mb-2 block">Largura</label>
            <div className={`flex gap-1 p-1 rounded-lg ${darkMode ? 'bg-black' : 'bg-gray-100'}`}>
              {['full', 'prose'].map(w => (
                <button
                  key={w}
                  onClick={() => onChange({ width: w })}
                  className={`flex-1 py-1 rounded text-[8px] font-black uppercase transition-all ${hh.width === w ? (darkMode ? 'bg-zinc-800 text-white shadow-sm' : 'bg-white text-black shadow-sm') : 'text-zinc-500'}`}
                >
                  {w === 'full' ? '100%' : '65ch'}
                </button>
              ))}
        ...
        <div className={`pt-4 border-t ${darkMode ? 'border-white/5' : 'border-gray-100'}`}>
          ...
          <input
            ...
            className={`w-full border-2 rounded-xl px-4 py-3 text-[11px] font-bold outline-none transition-colors ${darkMode ? 'bg-black border-white/10 text-white focus:border-red-600' : 'bg-gray-50 border-gray-100 text-zinc-900 focus:border-red-600'}`}
