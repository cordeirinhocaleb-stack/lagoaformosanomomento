...
export const ListPanel: React.FC<ListPanelProps> = ({ variant, data, onChange }) => {
  if (['bullets_clean', 'bullets_square', 'numbered_steps', 'timeline_dots'].includes(variant)) {
    ...
    return (
      <div className="space-y-8 animate-fadeIn">
            ...
            <div className="grid grid-cols-3 gap-2 bg-gray-100 p-1.5 rounded-2xl border border-gray-200">
                {[
                    ...
                    <button 
                        ...
                        className={`flex flex-col items-center justify-center py-3 rounded-xl transition-all ${ls.spacing === opt.val ? 'bg-white shadow-xl text-red-600 scale-105 z-10' : 'text-zinc-400 hover:text-zinc-600'}`}
                    >
            ...
            <div className="grid grid-cols-2 gap-2">
                {[
                    ...
                    <button
                        ...
                        className={`flex items-center gap-3 p-4 rounded-2xl border-2 transition-all ${ls.rowStyle === opt.val ? 'bg-black text-white border-black shadow-lg scale-[1.02]' : 'bg-white border-zinc-100 text-zinc-400 hover:bg-gray-50'}`}
                    >
        ...
        <div>
            ...
            <div className="flex gap-3 flex-wrap justify-center bg-zinc-50 p-4 rounded-[2rem] border border-zinc-100">
                {[
                    ...
                    <button
                        ...
                        className={`w-10 h-10 rounded-full ${c.color} transition-all border-4 ${ls.markerColor === c.val ? 'border-white ring-4 ring-zinc-300 scale-125 shadow-2xl z-20' : 'border-transparent opacity-40 hover:opacity-100'}`}
        ...
        <div className="grid grid-cols-2 gap-4 border-t border-gray-100 pt-6">
            <div className="flex flex-col gap-2">
                ...
                <div className="flex bg-gray-100 p-1 rounded-xl">
                    {['sm', 'normal', 'lg'].map(size => (
                        <button key={size} onClick={() => onChange({ fontSize: size })} className={`flex-1 py-2 rounded-lg text-[9px] font-black uppercase ${ls.fontSize === size ? 'bg-white shadow-sm text-zinc-900' : 'text-zinc-400'}`}>
            ...
            <div className="flex flex-col gap-2">
                ...
                <button 
                    ...
                    className={`flex-1 flex items-center justify-center gap-2 rounded-xl border-2 font-black text-[10px] uppercase transition-all ${ls.weight === 'bold' ? 'bg-red-600 text-white border-red-600 shadow-lg' : 'bg-white text-gray-400 border-zinc-100'}`}
                >
