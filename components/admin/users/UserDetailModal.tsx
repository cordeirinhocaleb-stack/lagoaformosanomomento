...
const UserDetailModal: React.FC<UserDetailModalProps> = ({
    ...
    newUserPassword, isCreating, adConfig, onSavePermissions, systemSettings, darkMode = false
}) => {
    ...
    return createPortal(
        <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4">
            ...
            {/* Modal Container */}
            <div className={`w-full max-w-6xl max-h-[90vh] rounded-[2rem] shadow-2xl relative z-10 animate-scaleIn flex overflow-hidden ${darkMode ? 'bg-[#0F0F0F]' : 'bg-white'}`}>
                ...
                <div className={`w-1/4 min-w-[280px] border-r flex flex-col ${darkMode ? 'bg-[#050505] border-white/5' : 'bg-gray-50 border-gray-200'}`}>
                    <div className={`p-8 flex flex-col items-center border-b ${darkMode ? 'bg-[#050505] border-white/5' : 'bg-gray-50 border-gray-200'}`}>
                        <div className={`w-24 h-24 rounded-3xl p-1 shadow-inner mb-4 group cursor-pointer transition-colors ${darkMode ? 'bg-white/5 hover:bg-white/10' : 'bg-white border border-gray-200 hover:bg-gray-100'}`}>
                            {user.avatar ? (
                                <img src={user.avatar} className="w-full h-full object-cover rounded-2xl group-hover:scale-105 transition-transform" />
                            ) : (
                        ...
                        <h2 className={`text-xl font-black text-center leading-none ${darkMode ? 'text-white' : 'text-gray-900'}`}>{user.name || 'Novo Usuário'}</h2>
                        <span className={`text-[10px] font-bold uppercase tracking-widest text-gray-500 mt-2 px-3 py-1 rounded-full ${darkMode ? 'bg-white/5' : 'bg-gray-200/50'}`}>{user.role}</span>

                        {/* Status Badges */}
                        <div className="flex gap-2 mt-4">
                            <span className={`px-2 py-1 rounded text-[9px] font-black uppercase ${user.isVerified ? 'bg-green-500/10 text-green-500' : 'bg-amber-500/10 text-amber-500'}`}>
                                {user.isVerified ? 'Verificado' : 'Pendente'}
                            </span>
                            <span className={`px-2 py-1 rounded text-[9px] font-black uppercase ${user.status === 'active' ? 'bg-blue-500/10 text-blue-500' : 'bg-red-500/10 text-red-500'}`}>
                    ...
                    <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
                        ...
                        <button onClick={() => setPanelTab('permissions')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${panelTab === 'permissions' ? (darkMode ? 'bg-white/10 text-white font-bold' : 'bg-white border border-gray-200 shadow-sm text-gray-900 font-bold') : 'text-gray-500 hover:bg-white/5 hover:text-gray-600'}`}>
                            <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${panelTab === 'permissions' ? 'bg-purple-600 text-white' : (darkMode ? 'bg-white/5 text-gray-600' : 'bg-gray-100 text-gray-400')}`}><i className="fas fa-lock"></i></div>
                        ...
                        <button onClick={() => setPanelTab('audit')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${panelTab === 'audit' ? (darkMode ? 'bg-white/10 text-white font-bold' : 'bg-white border border-gray-200 shadow-sm text-gray-900 font-bold') : 'text-gray-500 hover:bg-white/5 hover:text-gray-600'}`}>
                            <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${panelTab === 'audit' ? 'bg-orange-600 text-white' : (darkMode ? 'bg-white/5 text-gray-600' : 'bg-gray-100 text-gray-400')}`}><i className="fas fa-history"></i></div>
                ...
                <div className={`flex-1 overflow-y-auto p-8 custom-scrollbar relative ${darkMode ? 'bg-[#0F0F0F]' : 'bg-white'}`}>
                    {/* Notification Toast */}
                    {notification && (
                        <div className={`absolute top-4 right-4 z-50 p-3 rounded-xl shadow-lg flex items-center gap-3 animate-bounce-in ${notification.type === 'success' ? 'bg-green-600 text-white' : 'bg-red-600 text-white'}`}>
                    ...
                    <div className="pb-20">
                        {panelTab === 'control' && (
                            <>
                                ...
                                {!isCreating && <UserStatsPanel user={user} totalPosts={0} totalClicks={0} totalSpent={0} darkMode={darkMode} />}

                                <UserProfilePanel user={user} isCreating={isCreating} newUserPassword={newUserPassword} onUpdateUser={onUpdateUserLocal} onUpdatePassword={onUpdatePassword} onSave={onSaveUser} onToggleStatus={onToggleStatus} darkMode={darkMode} />

                                {!isCreating && <div className="mt-8"><UserPostsPanel userId={user.id} posts={[]} darkMode={darkMode} /></div>}
                            </>
                        )}

                        {panelTab === 'commercial' && (
                            <>
                                <UserSubscriptionPanel user={user} adConfig={adConfig} onUpdateUser={onUpdateUserLocal} onExtendSubscription={handleExtendSubscription} canEdit={canEditPlan} systemSettings={systemSettings} darkMode={darkMode} />
                                {user.role === 'Anunciante' && <UserFinancialPanel userId={user.id} payments={[]} darkMode={darkMode} />}
                            </>
                        )}

                        {panelTab === 'permissions' && (
                            <UserPermissionsPanel permissions={permissionsState} onTogglePermission={togglePermission} onSave={() => onSavePermissions(permissionsState)} darkMode={darkMode} />
                        )}
