...
const UserList: React.FC<UserListProps> = ({ users, onSelectUser }) => {
    return (
        <div className="w-full">
            ...
            {/* --- DESKTOP VIEW (TABLE) --- */}
            <div className="hidden md:block bg-white rounded-[2.5rem] border border-gray-100 shadow-sm overflow-hidden w-full">
                <table className="w-full text-left">
                    ...
                    <tbody className="divide-y divide-gray-50">
                        {users.map(u => (
                            <tr key={u.id} className="hover:bg-gray-50 transition-colors group cursor-pointer" onClick={() => onSelectUser(u)}>
                                <td className="p-6">
                                    <div className="flex items-center gap-4">
                                        <div className="relative">
                                            ...
                                            <div className={`absolute bottom-0 right-0 w-3 h-3 rounded-full border-2 border-white ${u.isVerified ? 'bg-blue-500' : 'bg-amber-500 animate-pulse'}`}></div>
                                        </div>
                                        <div>
                                ...
                                <td className="p-6">
                                    <span className={`px-3 py-1 rounded-lg text-[9px] font-black uppercase mb-1 inline-block ${u.role === 'Anunciante' ? 'bg-yellow-100 text-yellow-800' :
                                            u.role === 'Desenvolvedor' ? 'bg-zinc-900 text-white' :
                                ...
                                <td className="p-6 text-right">
                                    <button className="w-8 h-8 rounded-full bg-gray-100 hover:bg-black hover:text-white transition-colors flex items-center justify-center active:scale-90 shadow-sm border border-gray-200">
            ...
            <div className="md:hidden space-y-3">
                {users.map(u => (
                    <div
                        ...
                        className="bg-white rounded-[1.5rem] p-5 border border-gray-100 shadow-sm relative overflow-hidden active:scale-[0.98] transition-transform"
                    >
                        ...
                        <div className="flex justify-between items-start pl-2 mb-4">
                            <div className="flex items-center gap-3">
                                <div className="relative">
                                    <div className="w-12 h-12 rounded-full bg-gray-100 overflow-hidden border-2 border-white shadow-md flex items-center justify-center font-black text-gray-400 text-lg">
                                        {u.avatar ? <img src={u.avatar} className="w-full h-full object-cover" /> : u.name.charAt(0)}
                                    </div>
                                    <div className={`absolute -bottom-1 -right-1 w-5 h-5 rounded-full border-4 border-white flex items-center justify-center text-[8px] ${u.isVerified ? 'bg-blue-500 text-white' : 'bg-amber-400 text-black'}`}>
                                        <i className={`fas ${u.isVerified ? 'fa-check' : 'fa-exclamation'}`}></i>
                                    </div>
                                </div>
                                <div>
                                    <h4 className="font-black text-gray-900 text-base leading-tight">{u.name}</h4>
                                    <span className={`text-[8px] font-bold uppercase tracking-wider px-2 py-0.5 rounded mt-1 inline-block ${u.role === 'Anunciante' ? 'bg-yellow-50 text-yellow-700' :
                                            u.role === 'Desenvolvedor' ? 'bg-black text-white' :
                            ...
                            <button className="w-10 h-10 rounded-xl bg-gray-50 text-gray-400 flex items-center justify-center border border-gray-100 shadow-sm">
