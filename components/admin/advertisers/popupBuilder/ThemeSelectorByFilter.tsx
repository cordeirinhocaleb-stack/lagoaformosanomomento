...
const ThemeSelectorByFilter: React.FC<ThemeSelectorProps> = ({ currentThemeId, onSelect }) => {
    ...
    const filteredThemes = useMemo(() => {
        ...
        return POPUP_THEME_CATALOG.filter(t => t.id.startsWith(activeCategory) || t.id.includes(`_${activeCategory}`));
    }, [activeCategory]);
    ...
    return (
        <div className="flex h-[450px] border border-gray-100 rounded-[2rem] overflow-hidden bg-white shadow-sm ring-1 ring-black/5">
            {/* LEFT COLUMN: FILTERS (Menu Style) */}
            <div className="w-48 bg-gray-50/50 border-r border-gray-100 flex flex-col p-2 gap-1 overflow-y-auto">
                <div className="px-3 py-3 mb-2">
                    <span className="text-[10px] font-black uppercase text-gray-400 tracking-widest block">Estilos</span>
                </div>
                {THEME_CATEGORIES.map(cat => (
                    ...
                    >
                        <div className={`w-6 h-6 rounded-lg flex items-center justify-center text-xs transition-colors shrink-0 ${activeCategory === cat.id ? 'bg-black text-white' : 'bg-gray-200 text-gray-400 group-hover:bg-white'}`}>
                            ...
                            >
                                ...
                                <div 
                                    className={`
                                        ...
                                        ${isSelected ? 'ring-2 ring-red-600 shadow-xl' : 'group-hover:shadow-lg ring-1 ring-black/5'}
                                    `}
                                    style={{ 
                                        borderRadius: '16px',
                                        backgroundColor: '#e5e7eb', // Canvas bg
                                ...
                                <div className="flex flex-col items-start px-1">
                                    <span className={`text-[10px] font-black uppercase tracking-tight leading-none ${isSelected ? 'text-red-600' : 'text-gray-900'}`}>
                                        {theme.name}
                                    </span>
                                    <span className="text-[9px] text-gray-400 mt-1 line-clamp-1 text-left w-full">
