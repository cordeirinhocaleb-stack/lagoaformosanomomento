...
const BannersPanel: React.FC<BannersPanelProps> = ({ config, onChange }) => {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-8 h-[900px] animate-fadeIn">
        
        {/* LISTA LATERAL */}
        <div className="w-full lg:w-1/4 flex flex-col border-r border-gray-100 pr-6">
            <div className="flex justify-between items-center mb-4">
                <h3 className="text-xs font-black uppercase text-gray-400 tracking-widest">Banners ({banners.length})</h3>
                <button 
                    onClick={handleAdd}
                    className="bg-black text-white px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest hover:bg-green-600 transition-colors flex items-center gap-2"
                >
                ...
                {banners.map((banner) => {
                    ...
                    return (
                        <div 
                            ...
                            className={`p-3 rounded-xl border-2 cursor-pointer transition-all flex gap-3 items-center group ${editingId === banner.id ? 'border-red-600 bg-red-50' : 'border-gray-100 bg-white hover:border-gray-200'}`}
                        >
                            <div className="w-12 h-8 rounded bg-gray-200 overflow-hidden shrink-0 relative">
                                {thumb ? (
                            ...
                            <div className="flex-1 min-w-0">
                                <p className="text-[9px] font-black uppercase truncate text-gray-900">{banner.tag || 'Sem Tag'}</p>
                                <div className="flex gap-2 text-[8px] text-gray-400 items-center">
                            ...
                            <button 
                                ...
                                className="w-6 h-6 rounded-full bg-white text-gray-400 hover:text-red-600 hover:bg-red-50 flex items-center justify-center transition-colors opacity-0 group-hover:opacity-100 shadow-sm"
                            >
                    ...
                    <div className="space-y-6 flex-1">
                        <div className="flex justify-between items-center pb-2 border-b border-gray-100">
                            <div className="flex bg-gray-100 p-1 rounded-lg">
                                {['text', 'media', 'button'].map(tab => (
                                    <button
                                        ...
                                        className={`px-4 py-2 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${activeTab === tab ? 'bg-white shadow-sm text-black' : 'text-gray-400 hover:text-gray-600'}`}
                                    >
                            ...
                            <label className="flex items-center gap-2 cursor-pointer bg-gray-50 px-3 py-1.5 rounded-lg border border-gray-200">
                                ...
                                />
                                <div className={`w-3 h-3 rounded-full ${editingBanner.active ? 'bg-green-500' : 'bg-gray-300'}`}></div>
                                <span className="text-[9px] font-bold uppercase text-gray-500">{editingBanner.active ? 'Ativo' : 'Inativo'}</span>
                            </label>
                        </div>

                        {activeTab === 'text' && (
                            <BannerTextTab banner={editingBanner} onUpdate={(u) => handleUpdate(editingBanner.id, u)} />
                        )}

                        {activeTab === 'media' && (
                            <BannerMediaTab banner={editingBanner} onUpdate={(u) => handleUpdate(editingBanner.id, u)} />
                        )}

                        {activeTab === 'button' && (
                            <BannerButtonTab banner={editingBanner} onUpdate={(u) => handleUpdate(editingBanner.id, u)} />
                        )}
                    </div>
                </>
            ) : (
