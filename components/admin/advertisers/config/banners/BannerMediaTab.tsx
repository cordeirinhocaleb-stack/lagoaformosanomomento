...
export const BannerMediaTab: React.FC<BannerMediaTabProps> = ({ banner, onUpdate }) => {
        ...
        <div className="space-y-6 animate-fadeIn">
            <div className="flex gap-4 mb-4">
                <button
                    ...
                    className={`flex-1 py-3 rounded-xl border-2 flex items-center justify-center gap-2 transition-all ${banner.type === 'image' ? 'border-red-600 bg-red-50 text-red-600' : 'border-gray-100 text-gray-400'}`}
                >
                    <i className="fas fa-images"></i> Imagens
                </button>
                <button
                    onClick={() => onUpdate({ type: 'video' })}
                    className={`flex-1 py-3 rounded-xl border-2 flex items-center justify-center gap-2 transition-all ${banner.type === 'video' ? 'border-red-600 bg-red-50 text-red-600' : 'border-gray-100 text-gray-400'}`}
                >
            ...
            {banner.type === 'image' ? (
                <div className="space-y-4">
                    <div className="bg-gray-50 p-4 rounded-xl border border-gray-100 flex items-center justify-between">
                        <div>
                            <h4 className="text-[10px] font-black uppercase text-gray-900">Galeria (Slideshow)</h4>
                        ...
                        <div className="w-32 h-8 relative">
                            ...
                            <button className="w-full h-full bg-black text-white rounded-lg text-[9px] font-black uppercase tracking-widest flex items-center justify-center gap-2">
                    ...
                    <div className="grid grid-cols-3 gap-3">
                        {banner.images && banner.images.length > 0 ? (
                            banner.images.map((img, i) => (
                                <div key={i} className="relative aspect-video rounded-xl overflow-hidden group bg-gray-100 border border-gray-200">
                            ...
                            <div className="col-span-3 text-center py-4 text-xs text-gray-400 border-2 border-dashed border-gray-200 rounded-xl">Nenhuma imagem</div>
                        )}
                    </div>
                </div>
            ) : (
                <div className="space-y-4">
                    <div className="bg-gray-50 p-4 rounded-xl border border-gray-100">
                        <h4 className="text-[10px] font-black uppercase text-gray-900 mb-2">Arquivo de Vídeo</h4>
                        <div className="h-40 relative border-2 border-dashed border-gray-200 rounded-xl overflow-hidden group">
                            {banner.videoUrl ? (
