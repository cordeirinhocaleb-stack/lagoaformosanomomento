...
const ProductsSection: React.FC<ProductsSectionProps> = ({ data, onChange, darkMode = false }) => {
    ...
    const emptyStateClass = `text-center py-12 border-2 border-dashed rounded-2xl ${darkMode ? 'bg-white/5 border-white/10' : 'bg-gray-50 border-gray-200'}`;
    const cardClass = `border rounded-2xl p-4 flex flex-col md:flex-row gap-6 shadow-sm transition-all relative group ${darkMode ? 'bg-white/5 border-white/10 hover:border-white/20' : 'bg-white border-gray-200 hover:border-gray-300'}`;
    const inputClass = `w-full rounded-lg px-3 py-2 text-sm font-bold outline-none border transition-colors ${darkMode ? 'bg-black/20 border-white/5 text-white focus:border-white/20' : 'bg-gray-50 border-gray-200 focus:border-black'}`;
        ...
        <div className="space-y-6 animate-fadeIn">
            <div className="flex justify-between items-center mb-6">
                ...
                <button
                    ...
                    className={`px-4 py-2 rounded-lg text-[9px] font-black uppercase tracking-widest transition-colors flex items-center gap-2 ${darkMode ? 'bg-white text-black hover:bg-gray-200' : 'bg-black text-white hover:bg-gray-800'}`}
                >
                ...
                <div className="grid grid-cols-1 gap-6">
                    {products.map((product, idx) => (
                        <div key={product.id} className={cardClass}>
                            ...
                            <div className={`w-full md:w-32 aspect-square rounded-xl overflow-hidden shrink-0 border ${darkMode ? 'bg-white/5 border-white/5' : 'bg-gray-50 border-gray-100'}`}>
                                {product.imageUrl ? (
                            ...
                            <div className="flex-1 space-y-3">
                                <div className="flex gap-4">
                                    ...
                                    <div className="w-32">
                                        ...
                                        <input
                                            ...
                                            className={`w-full rounded-lg px-3 py-2 text-sm font-bold outline-none border transition-colors ${darkMode ? 'bg-black/20 border-white/5 text-green-400' : 'bg-gray-50 border-gray-200 text-green-700'}`}
                                            placeholder="0,00"
                                        />
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        className={`w-full rounded-lg px-3 py-2 text-xs outline-none border transition-colors ${darkMode ? 'bg-black/20 border-white/5 text-gray-300' : 'bg-gray-50 border-gray-200 text-gray-700'}`}
                                ...
                                <div className="flex items-center gap-4 pt-2">
                                    <label className="flex items-center gap-2 cursor-pointer">
                                        <input
                                            ...
                                            className={`focus:ring-black ${darkMode ? 'text-white' : 'text-black'}`}
                                        />
                                        <span className={`text-[10px] font-bold uppercase ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>Padrão</span>
                                    </label>
                                    <label className="flex items-center gap-2 cursor-pointer">
                                        <input
                                            type="radio"
