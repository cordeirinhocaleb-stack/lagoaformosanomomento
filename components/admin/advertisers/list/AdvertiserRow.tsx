...
const AdvertiserRow: React.FC<AdvertiserRowProps> = ({ advertiser, onEdit }) => {
  ...
  return (
    <div 
        ...
        className="group bg-white p-6 md:p-8 rounded-[2.5rem] border border-gray-100 shadow-sm hover:shadow-xl hover:border-red-100 transition-all cursor-pointer relative overflow-hidden flex flex-col lg:flex-row items-center gap-6"
    >
      <div className={`absolute left-0 top-0 bottom-0 w-2 ${advertiser.isActive && !isExpired ? 'bg-green-500 shadow-[2px_0_10px_rgba(34,197,94,0.3)]' : 'bg-gray-200'} group-hover:bg-red-600 transition-colors`}></div>

      <div className="w-20 h-20 rounded-3xl bg-gray-50 border border-gray-100 flex items-center justify-center shrink-0 overflow-hidden group-hover:scale-105 transition-transform shadow-inner">
        {advertiser.logoUrl ? (
            <img src={advertiser.logoUrl} alt={advertiser.name} className="w-full h-full object-cover" />
        ) : (
            <i className={`fas ${advertiser.logoIcon || 'fa-store'} text-3xl text-gray-300 group-hover:text-red-500 transition-colors`}></i>
        )}
      </div>
      ...
      <div className="flex-1 text-center lg:text-left min-w-0 w-full">
        <div className="flex flex-col lg:flex-row lg:items-center gap-3 mb-2">
            <h3 className="text-xl font-black uppercase text-gray-900 truncate tracking-tight group-hover:text-red-600 transition-colors">
                {advertiser.name}
            </h3>
        ...
        <p className="text-[11px] font-bold text-gray-400 uppercase tracking-widest flex flex-col lg:flex-row gap-2 lg:gap-6 items-center lg:items-start">
            <span className="flex items-center gap-1.5"><i className="fas fa-tag text-gray-300"></i> {advertiser.category}</span>
      ...
      <div className="flex flex-col gap-5 w-full lg:w-72 border-t lg:border-t-0 lg:border-l border-gray-100 pt-5 lg:pt-0 lg:pl-6">
        <div>
            ...
            <div className="flex justify-between text-[8px] font-bold uppercase text-gray-400 mt-2">
                <span className="bg-gray-50 px-2 py-0.5 rounded border">{startDate.toLocaleDateString()}</span>
                <span className="bg-gray-50 px-2 py-0.5 rounded border">{endDate.toLocaleDateString()}</span>
            </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-2">
            <div className="bg-gray-50 rounded-xl p-3 text-center border border-gray-100 shadow-sm">
                ...
                <span className="block text-sm font-black text-gray-900">{advertiser.views}</span>
            </div>
            <div className="bg-gray-50 rounded-xl p-3 text-center border border-gray-100 shadow-sm">
                <span className="block text-[9px] font-black uppercase text-gray-400 mb-0.5 tracking-tighter">Clicks</span>
                <span className="block text-sm font-black text-gray-900">{advertiser.clicks}</span>
            </div>
            <div className="bg-blue-50 rounded-xl p-3 text-center border border-blue-100 shadow-sm">
                ...
                <span className="block text-sm font-black text-blue-600">{ctr}%</span>
            </div>
        </div>
      </div>

      <div className="flex lg:flex pl-4 lg:border-l border-gray-100 w-full lg:w-auto">
        <button 
            className="w-full lg:w-14 h-14 rounded-2xl bg-black text-white hover:bg-red-600 transition-all flex items-center justify-center shadow-xl active:scale-95 group/btn"
